...
let tempFolder = 'temp/etc/designs/kaousa/common/';
let targetFolder = 'target/etc/designs/kaousa/common/';
let designPath = 'etc/designs/kaousa/common/';
let subDesignPath = 'etc/designs/kaousa/common/sub/';
let clientLibsToMinify = ['blog'];
...
gulp.task('deploy-css', function () {
    ...
    if (sub == undefined || sub == null || sub == "") {
        return run('curl --fail -T {' + cssFileNamesArray + '} ' + target + designPath + 'blog/css/').exec();
    }
    else {
        return run('curl --fail -T {' + cssFileNamesArray + '} ' + target + subDesignPath + sub + '/blog/css/').exec();
    }
...
gulp.task('watch-deploy', function () {
    gulp.watch('blog/scss/**/*.scss', ['run-seq-for-css']);
    gulp.watch(['*/*.js', '*/js/*.js', '*/js/*/*.js']).on('change', function (file) {
        ...
        if (sub == undefined || sub == null || sub == "") {
            ...
            return run('curl --fail -T ' + path + ' ' + target + subDesignPath + sub + '/blog/js/').exec();
        }
    });
});
