...
let tempFolder = 'temp/etc/designs/kaousa/www-johnfrieda-com/';
let targetFolder = 'target/etc/designs/kaousa/www-johnfrieda-com/';
let designPath = 'etc/designs/kaousa/www-johnfrieda-com/';
let clientLibsToMinify = ['newsletter-signup', 'newsletter-subscribe', 'basic', 'component', 'birthdaycoupon', 'option', 'newsletter-unsubscribe'];
...
//@elb-ver-wcm-auth-472389826.ap-northeast-1.elb.amazonaws.com
...
gulp.task('compass', function () {
    return gulp.src('./_scss/component/css/*.scss')
        .pipe(compass({
...
gulp.task('deploy-css', function () {
    ...
    return run('curl -T {' + cssFileNamesArray + '} ' + target + designPath + 'component/css/').exec();
});
...
gulp.task('watch-deploy', function () {
    gulp.watch('_scss/**/*.scss', ['run-seq-for-css']);
