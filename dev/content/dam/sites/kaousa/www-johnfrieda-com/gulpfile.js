 ...
 * gulp-run, fancy-log, gulp-if, gulp-replace-name.
 * 
 * (or)
 * 
...
let tempFolder = 'temp/content/dam/sites/kaousa/www-johnfrieda-com/';
let tempFolder2 = 'temp/content/dam/sites/kaousa/www-johnfrieda-com/master/';
let targetFolder = 'target/content/dam/sites/kaousa/www-johnfrieda-com/';
let designPath = 'content/dam/sites/kaousa/www-johnfrieda-com/';
let clientLibsToMinify = ['master/freehtml','jfman'];
...
if (env == 'sit'){
    target = 'http://cogadmin:vO1gFT9PXMH6@elb-stg-guhl-sit-an1-cts-sp2-2086753287.ap-northeast-1.elb.amazonaws.com/';
}else if(env == 'prod'){
 ...
 */
gulp.task('copy-js-files-to-temp',['clean-temp'], function() {
...
gulp.task('copy-css-files-to-temp', function(){
    return gulp.src(['./**/*.css','!node_modules/**'])
    //.pipe(rename({dirname: 'master/freehtml/css/*/'}))
    .pipe(gulp.dest(tempFolder));
});
...
gulp.task('copy-files-to-target',['clean-target'], function() {
    return gulp.src([tempFolder  + '/**/*.css',tempFolder + '/**/*.js',
        tempFolder + '/**/*.js.map'])
 ...
 * 
 * All JS files are minified individually and minified file and map file are generated one
 * per JS file
 * 
 * Original files are suffixed with '.original.js' and map files are suffixed with '.js.map'.
 */
...
gulp.task('build', function() {
    if(minificationRequired) {
        runSequence('copy-js-files-to-temp','copy-css-files-to-temp', 'minify-css', 'minify-js', 'copy-files-to-target',
            function() {
                // do nothing
            }
        );
    } else {
        runSequence('copy-js-files-to-temp', 'copy-css-files-to-temp','copy-files-to-target',
            function() {
                // do nothing
            }
        );
...
gulp.task('watch-deploy', function () {
    gulp.watch(['**/*.css','**/*.js']).on('change',function(file) {
